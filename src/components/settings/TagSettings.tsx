...
export function TagSettings() {
  ...
  const saveEdit = () => {
    if (editingId && editName.trim()) {
      updateTag(editingId, { name: editName.trim().toLowerCase(), color: editColor });
    }
    setEditingId(null);
  };
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-xl font-semibold text-[var(--color-text-primary)] mb-4">Tags</h2>
        <p className="text-sm text-[var(--color-text-secondary)] mb-6">
          Manage the tags available for your task cards. Each tag has a name and color.
        </p>
      </div>
      ...
      <div className="flex gap-3 items-end p-4 bg-[var(--color-bg-tertiary)] rounded-lg border border-[var(--color-border)]">
        ...
        <div>
          <label className="block text-sm font-medium text-[var(--color-text-primary)] mb-1">Color</label>
        ...
        <Button onClick={handleAdd} disabled={!newName.trim()} leftIcon={<Plus size={16} />}>
          Add
        </Button>
      </div>
      ...
      <div className="space-y-2">
        {tags.length === 0 ? (
          <p className="text-center py-8 text-[var(--color-text-tertiary)] text-sm">No tags defined yet</p>
        ) : (
          tags.map((tag) => (
            <div key={tag.id} className="flex items-center justify-between p-3 rounded-lg border border-[var(--color-border)] hover:bg-[var(--color-surface-hover)] transition-colors">
              {editingId === tag.id ? (
                <div className="flex items-center gap-2 flex-1">
                  ...
                  <button onClick={saveEdit} className="p-1 hover:bg-green-100 dark:hover:bg-green-900/30 rounded">
                ...
                <>
                  ...
                  <div className="flex gap-2">
                    <button
                      ...
                      className="text-xs text-[var(--color-text-secondary)] hover:text-[var(--color-text-primary)] px-2 py-1 rounded hover:bg-[var(--color-surface-active)] transition-colors"
                    >
                      Edit
                    </button>
                    <button
